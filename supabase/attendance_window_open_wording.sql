...
-- reminder to the recipient. Reframed as "Booking with [Name] for '[Service]':
-- ..." so the name is context, and the actor_id/bold-name prefix is
-- suppressed for this type in NotificationsPanel.tsx.
--
  ...
  if found then
    ...
    insert into public.notifications (user_id, actor_id, type, title, message, related_id, read) values
      (v_booking.client_id, v_booking.freelancer_id, 'attendance_window_open',
       ...
       format('Booking with %s for ''%s'': check-in is now open — please confirm you''ve seen them at the location.',
         coalesce(v_freelancer_name, 'your freelancer'), v_booking.project_name),
       p_booking_id, false),
      (v_booking.freelancer_id, v_booking.client_id, 'attendance_window_open',
       'Attendance check available',
       format('Booking with %s for ''%s'': check-in is now open — please confirm you''ve seen them at the location.',
         coalesce(v_client_name, 'your client'), v_booking.project_name),
       p_booking_id, false);
    return v_updated;
  end if;
