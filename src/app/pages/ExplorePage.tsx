...
import { Search, ChevronLeft, ChevronRight, SlidersHorizontal, Star, Sparkles, Heart, Crown, Lock } from 'lucide-react';
    ...
    actions: (
      <div className={`flex flex-shrink-0 items-center gap-3 ${condensedVisibilityClass}`}>
        ...
        <button
          ...
          title={user?.isPremium ? 'Event Assistant — Premium feature' : 'Event Assistant — Premium feature, locked'}
          tabIndex={isPastSearchSection ? 0 : -1}
          className={`relative flex flex-shrink-0 items-center gap-1.5 rounded-full px-2.5 py-2 text-xs font-semibold text-white shadow-md transition-transform hover:scale-105 @sm:px-3 ${
            user?.isPremium ? 'bg-gradient-to-r from-cyan-500 to-indigo-600 shadow-cyan-500/30' : 'bg-gradient-to-r from-gray-400 to-gray-500 shadow-gray-400/30'
          }`}
        >
    ...
    mobileActions: (
      <div className={`flex items-center gap-1.5 ${condensedVisibilityClass}`}>
        ...
        <button
          ...
          onClick={() => navigate('/event-matcher')}
          title={user?.isPremium ? 'Event Assistant — Premium feature' : 'Event Assistant — Premium feature, locked'}
          aria-label={user?.isPremium ? 'Event Assistant — Premium feature' : 'Event Assistant — Premium feature, locked'}
          tabIndex={isPastSearchSection ? 0 : -1}
          className={`relative flex h-8 w-8 items-center justify-center rounded-full text-white shadow-md transition-transform hover:scale-105 ${
            user?.isPremium ? 'bg-gradient-to-r from-cyan-500 to-indigo-600 shadow-cyan-500/30' : 'bg-gradient-to-r from-gray-400 to-gray-500 shadow-gray-400/30'
          }`}
        >
          {user?.isPremium ? <Sparkles className="h-4 w-4" /> : <Lock className="h-4 w-4" />}
      ...
      <div ref={searchSectionRef} className="flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-4 mb-8 md:mb-12">
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() => navigate('/event-matcher')}
            className={`relative flex-1 md:flex-none flex items-center gap-3 px-4 md:px-6 py-3 md:py-4 bg-white/80 backdrop-blur-xl rounded-2xl shadow-[0_8px_24px_rgba(56,189,248,0.18)] hover:shadow-[0_12px_32px_rgba(56,189,248,0.3)] hover:-translate-y-0.5 transition-all group ${
              user?.isPremium ? 'border border-sky-100' : 'border border-amber-200 bg-amber-50/30'
            }`}
          >
            <div
              className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-transform ${
                user?.isPremium
                  ? 'bg-gradient-to-br from-cyan-400 to-indigo-500 shadow-lg shadow-cyan-500/40'
                  : 'bg-gradient-to-br from-gray-400 to-gray-500 shadow-lg shadow-gray-400/40'
              }`}
            >
              {user?.isPremium ? <Sparkles className="w-5 h-5 md:w-6 md:h-6 text-white" /> : <Lock className="w-5 h-5 md:w-6 md:h-6 text-white" />}
            </div>
            <div className="text-left">
