...
export function ForYouPage({ onViewProfile, onOpenMessages }: ForYouPageProps) {
  ...
  // that still needs its own picker UI, since it needs a mutuals list) is
  // folded in as an extra item inside this same menu via handleShare,
  // which now does nothing BUT open that picker.
  const buildShareMenu = (post: FeedPost, variant: 'card' | 'modal') => (
    <PostShareMenu
      postId={post.id.replace(/^client-post-/, '')}
      title={`CreativeHUB post by @${post.username}`}
      ...
      onShared={(method: PostShareMethod) => {
        if (user?.id && post.isClientPost) {
          void DataService.recordClientPostShare(user.id, post.id.replace(/^client-post-/, ''), method);
        }
      }}
      extraItems={(close) => (
        <>
