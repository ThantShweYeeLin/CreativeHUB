...
import { AlertCircle, Ban, CheckCircle, Search, Shield, Users as UsersIcon } from 'lucide-react';
...
type Tab = 'users' | 'reports' | 'tickets' | 'disputes';
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-lg border-b border-gray-200">
        <div className="max-w-[1200px] mx-auto px-4 md:px-8 py-4">
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto">
            {(['users', 'reports', 'tickets', 'disputes'] as Tab[]).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
              ...
              >
                {t === 'users' ? 'Users' : t === 'reports' ? 'Reports' : t === 'tickets' ? 'Tickets' : 'Disputes'}
              </button>
            ))}
          </div>
        </div>
...
function DisputesTab() {
  ...
  const load = async () => {
    setIsLoading(true);
    const response = await DataService.getAllDisputedBookingsForAdmin();
    if (response.error) {
      setError((response.error as any).message || 'Unable to load disputes.');
      setIsLoading(false);
      return;
    }
