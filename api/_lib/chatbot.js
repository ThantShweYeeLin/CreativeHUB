// Support chatbot logic, shared by both places the chat endpoint runs:
//   - api/chatbot/message.js — the Vercel function on the live website
//     (same project as the frontend, so no separate backend deploy needed)
//   - server/src/routes/chatbot.ts — the Express server used in local dev
// Plain JS with zero dependencies beyond @supabase/supabase-js (already a
// root dependency); the AI providers are called over REST with fetch. The
// leading underscore keeps Vercel from turning this file into an endpoint.
//
